...
/* A raw file input paints an OS button in the middle of a LaTeX page. The
   input still exists and is still what the browser opens; only the chrome is
   replaced, with the same small-caps the nav and the submit button use. */
.cvline input[type=file] { position:absolute; width:1px; height:1px; opacity:0;
  overflow:hidden; clip:rect(0 0 0 0); }
.cvline label.pick { font-variant:small-caps; border-bottom:.8px solid var(--ink);
  cursor:pointer; padding-bottom:.05rem; }
.cvline label.pick:hover { color:var(--link); border-bottom-color:var(--link); }
#cv-msg { font-style:italic; }
...
FORM_HTML = """<div class="algo" id="join">
  ...
  <form id="join-form">
    ...
    <div class="field"><label></label><div id="reach" class="tabnote"></div></div>
    <div class="cvline">or read a CV and it fills the box above:
      <label class="pick" for="f-cv">choose a file</label>
      <input id="f-cv" type="file" accept=".pdf,.txt,.md">
      <span id="cv-msg"></span><br>
      <span class="tex">the file is read in this tab and never uploaded.</span>
      <a href="cv.html">see the full report it can write</a></div>
...
/* LIVE REACH. Type "seks" and today that silently means zero mail, forever,
   with nothing on the page saying so -- same for a typo, a single letter, or
   an empty box. The count below is a promise, so it uses the MAILER's rule:
   engine/build_site.py ships the titles already normalised by match.norm_title
   and the matcher's own expansion table, and this code only splits and does a
   substring test. A second normaliser written here would drift and start
   promising numbers the engine does not deliver.
   Loaded on first touch, because it is bigger than the page and most visitors
   never type in that box. */
let boardReady = null;
function loadBoard() {
  if (boardReady) return boardReady;
  boardReady = new Promise((ok, no) => {
    const s = document.createElement('script');
    s.src = 'board.js'; s.onload = ok; s.onerror = no;
...
async function showReach() {
  const raw = interests.value.trim();
  if (!raw) { reach.textContent = ''; return; }
  reach.textContent = 'counting…';
  try { await loadBoard(); } catch (_) { reach.textContent = ''; return; }
  const kws = keywords(raw);
  let n = 0;
  for (const t of BOARD) if (kws.some(k => t.includes(' ' + k + ' '))) n++;
  reach.textContent = n
    ? n + ' of today\u2019s ' + BOARD.length + ' listings match that. new ones arrive every morning.'
    : 'nothing on today\u2019s ' + BOARD.length + ' listings matches that \u2014 you would get no mail. try a word that appears in job titles.';
}
interests.addEventListener('input', () => {
  clearTimeout(reachTimer); reachTimer = setTimeout(showReach, 250);
});

/* CV -> interests. The form used to say "upload a CV and the profile fills
   itself" while having no file input and no code that filled anything: a
   promise made to every visitor and kept for none. This keeps it.
   Nothing is uploaded. The file is read in this tab, exactly as cv.html does
   it, and only the words it found are put in a text box the user can edit.
   pdf.js is 1.4 MB and this page is 11 kB, so it is fetched only if a PDF is
   actually chosen -- and cv-engine.js only when a file is. */
...
document.getElementById('f-cv').addEventListener('change', async e => {
  const file = e.target.files && e.target.files[0];
  if (!file) return;
  cvMsg.textContent = ' reading…';
  try {
    const text = /\\.pdf$/i.test(file.name) ? await readPdf(file) : await file.text();
    if (!window.cvEvidence) await loadScript('cv-engine.js');
    const ev = cvEvidence(text);
    const found = (ev.terms || []).slice(0, 6);
    if (!found.length) {
      cvMsg.textContent = ' read it, but found no field words in it. type them yourself.';
      return;
    }
    interests.value = found.join(', ');
    cvMsg.textContent = ' filled from the CV \u2014 edit it if it is wrong.';
    showReach();
  } catch (err) {
    cvMsg.textContent = ' could not read that file. type your interests instead.';
  }
});
...
def write_board(root, jobs):
    """The board's titles, ALREADY NORMALISED by match.norm_title.

    The signup form counts matches live, and that count is a promise: type
    "seks" and today it silently means zero mail, forever, with nothing on the
    page saying so. To be a promise and not a guess it has to use the mailer's
    rule -- so the normalisation happens HERE, in the same function the matcher
    calls, and the browser only does a substring test. A second normaliser
    written in JavaScript would drift from this one and start promising numbers
    the engine does not deliver.

    Loaded lazily, on first touch of the interests field: it is bigger than the
    page and most visitors never type in that box.
    """
    titles = [match.norm_title(j["position"]) for j in jobs]
    # The expansion table travels WITH the titles instead of being retyped in
    # JavaScript. It is the matcher's own dict, read at build time: "ai infra"
    # is not one keyword, it is six, and a preview that did not know that would
    # under-count every profile that uses the placeholder's own wording.
    (root / "board.js").write_text(
        "const BOARD = " + json.dumps(titles, ensure_ascii=False) + ";\n"
        "const EXPANSION = " + json.dumps(match.INTEREST_EXPANSION, ensure_ascii=False) + ";\n"
        "const SPLIT = " + json.dumps(match.INTEREST_SPLIT) + ";\n"
        "const KWMIN = " + str(match.INTEREST_MIN) + ", KWMAX = " + str(match.INTEREST_MAX) + ";",
        encoding="utf-8")
